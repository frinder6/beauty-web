		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#easyui-menu-self').find('.click-class').on('click', function(){
					...
					var exists = $(cid).tabs('exists', $(this).attr('data-value'));
					if (exists) {
						$(cid).tabs('select', $(this).attr('data-value'));
					} else {
						$(cid).tabs('add', {
			                title : $(this).attr('data-value'),
			                closable : true,
			                href : _PATH($(this).attr('data-url'))
			            });
					}
				});
			}
		...
		$.ajax({
			...
			success : function(data) {
				...
				hnav.find('.linkbutton').on('click', function() {
					...
					// 关闭标签页
					var tabs = $(cid).tabs('tabs');
					var ts = $.map(tabs, function(tab, i){
						var t = $(tab).panel('options').tab.text();
						if (t != 'Welcome'){
							return t;
						}
					});
